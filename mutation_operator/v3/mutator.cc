    ...
    for (int i = 0; i < field_count; ++i) {
      ...
      if (const OneofDescriptor* oneof = field->containing_oneof()) {
        ...
        if (field->is_repeated()) {
          ...
          Try({message, field, random_index}, Mutation::MultipleClone);
...
bool Mutator::MutateImpl(const ConstMessages& sources, const Messages& messages,
  ...
  if (copy_clone_only) {
    ...
    mutations[static_cast<size_t>(Mutation::MultipleClone)] = true;
  ...
  if (!copy_clone_only && std::uniform_int_distribution<uint64_t>(1, 100)(random_) <= 80/* 80% for Mutate*/) {
    mutations.reset();
    mutations[static_cast<size_t>(Mutation::Mutate)] = true;
  } else {
    mutations[static_cast<size_t>(Mutation::Mutate)] = false;
  }
  ...
  while (mutations.any()) {
    ...
    switch (mutation.mutation()) {
      ...
      case Mutation::MultipleClone: {
        CreateDefaultField()(mutation.field());
        DataSourceSampler source_sampler(mutation.field(), &random_,
                                         size_increase_hint);
        for (const Message* source : sources) source_sampler.Sample(*source);

	const protobuf::Message* src_message = source_sampler.field().get_message();
	const protobuf::Reflection& src_reflection = source_sampler.field().reflection();

	int src_field_size;
	if (src_message) {
	  src_field_size = src_reflection.FieldSize(*src_message, source_sampler.field().descriptor());
	}
	else {
	  break;
	}

	int mut_idx = mutation.field().index();
	int src_idx = source_sampler.field().index();
	int possible_n = src_field_size - src_idx + 1;
	int clone_n = GetRandomIndex(&random_, possible_n-1)+1; 

        DeleteField()(mutation.field());
	FieldInstance last_mut = mutation.field();;
	ConstFieldInstance last_src = source_sampler.field();

	for (int i = 0; i < clone_n; i++) {

          CreateDefaultField()(last_mut);
          CopyField()(last_src, last_mut);

	  last_mut = mutation.field().Clone(++mut_idx);
	  last_src = source_sampler.field().Clone(++src_idx);
	}
	return true;
