...
if [ ! -f "./fuzz_print" ]; then
	...
	N=$(python3 /graphfuzz/harfbuzz/gfuzz/script/sub/parse_seq_len.py ./schema.json)
	python3 /graphfuzz/harfbuzz/gfuzz/script/sub/gen_printer.py $N > fuzz_printer.cpp
	clang++-12 \
    		fuzz_printer.cpp \
    		-fsanitize=address \
